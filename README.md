...
This is the source code repository for the `pagan`
VS Code extension.
...
3. Run the `yarn ovsx publish` command with the correct
   ...
   ```bash
   yarn ovsx publish "extension/pagan-$(jq -r .version extension/package.json).vsix"
   ```
...
vscode-pagan depends only indirectly, e.g. through a
...
all the way down to vscode-pagan), the patch may be
